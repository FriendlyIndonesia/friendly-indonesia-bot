...
✅ Saldo otomatis masuk setelah bayar!`,
  ...
  if (sessions[id]?.__ppobStep === "input_nomor") {
    ...
    const btns = displayed.map((p, i) => [{ text: `${p.product_name} — ${fmt(p.price)}`, callback_data: `pX.${type}.${i}.${nomorEncoded}` }]);
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("pXp.")) {
    ...
    const btns = displayed.map((p, i) => [{ text: `${p.product_name} — ${fmt(p.price)}`, callback_data: `pX.${type}.${page * PAGE_SIZE + i}.${nomorEnc}` }]);
    const navRow = [];
