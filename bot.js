...
async function createMidtransPayment({ orderId, amount, customerName, itemName }) {
  ...
  const payload = JSON.stringify({
    ...
    item_details: [{ id:orderId.substring(0,50), price:amount, quantity:1, name:itemName.substring(0,50) }],
    callbacks: { finish:"https://t.me/" + botUser }
  });
  try {
    const res = await fetchJSON(MT_BASE + "/transactions", {
      method:"POST",
      headers:{ "Authorization":"Basic " + auth, "Content-Type":"application/json" },
      body: payload
    });
    if (!res?.redirect_url) console.error("Midtrans response:", JSON.stringify(res));
    return res;
...
✅ Saldo otomatis masuk setelah bayar!`,
  ...
  if (sessions[id]?.__ppobStep === "input_nomor") {
    ...
    const btns = displayed.map((p, i) => [{ text: `${p.product_name} — ${fmt(p.price)}`, callback_data: `px_${type}_${i}_${nomorEncoded}` }]);
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("pxp_")) {
    ...
    const [,type, pageStr, nomorRaw] = data.split("_");
    ...
    const btns = displayed.map((p, i) => [{ text: `${p.product_name} — ${fmt(p.price)}`, callback_data: `px_${type}_${page * PAGE_SIZE + i}_${nomorEnc}` }]);
    const navRow = [];
    if (page > 0) navRow.push({ text:`⬅️ Hal ${page}`, callback_data:`pxp_${type}_${page-1}_${nomorEnc}` });
    if (page < totalPages - 1) navRow.push({ text:`➡️ Hal ${page+2}/${totalPages}`, callback_data:`pxp_${type}_${page+1}_${nomorEnc}` });
