...
async function digiGetProduk(kategori) {
  ...
  const katMap = {
    "pulsa":    c => /pulsa/i.test(c),
    "data":     c => /paket data|data/i.test(c) && !/pulsa/i.test(c),
    "pln":      c => /pln|listrik|token/i.test(c),
    "bpjs":     c => /bpjs/i.test(c),
    "game":     c => /game|voucher game|top.?up game/i.test(c),
    "ewallet":  c => /e.?money|e.?wallet|gopay|ovo|dana|shopeepay/i.test(c),
    "tv":       c => /tv|televisi|kabel/i.test(c),
    "internet": c => /internet|wifi|broadband/i.test(c),
  };
