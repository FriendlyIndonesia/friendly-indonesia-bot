...
function hashPin(pin) {
  return crypto_builtin.createHash("sha256").update(pin + "friendlyindonesia").digest("hex");
}

function verifyPin(userId, inputPin) {
  const storedHash = users[userId]?.pin;
  if (!storedHash) return null; // belum set PIN
  return storedHash === hashPin(inputPin);
}

// pendingPinAction: { userId: { action, data, expiry } }
const pendingPinAction = {};

async function requirePin(userId, chatId, actionCallback, actionData) {
  const user = users[userId];
  // Kalau belum set PIN, langsung proses tanpa PIN
  if (!user?.pin) return actionCallback(actionData);

  // Simpan action yang menunggu PIN
  pendingPinAction[userId] = {
    callback: actionCallback,
    data:     actionData,
    expiry:   Date.now() + 120000, // 2 menit
    attempts: 0
  };
...
` +
    `Ketik 6 digit PIN kamu untuk melanjutkan.
` +
    `_Sesi berlaku 2 menit_`,
    { parse_mode:"Markdown", reply_markup:{ inline_keyboard:[
      [{ text:"❌ Batal", callback_data:"cancel_pin" }]
    ]}}
  );
}
...
bot.on("message", async (msg) => {
  ...
  if (text === "⭐ Poin & Reward") {
    ...
    return bot.sendMessage(id,
      ...
      { parse_mode:"Markdown", reply_markup:{ inline_keyboard:[
        [{ text: users[id]?.pin ? "🔐 Ubah/Hapus PIN" : "🔐 Set PIN Transaksi", callback_data:"pin_menu" }],
        [{ text:"🏠 Menu Utama", callback_data:"back_main" }]
      ]}}
    );
  }
  ...
  // ── PIN INPUT HANDLER ──
  if (pendingPinAction[id]) {
    const pa = pendingPinAction[id];
    if (Date.now() > pa.expiry) {
      delete pendingPinAction[id];
      return bot.sendMessage(id, "⏰ Sesi PIN expired. Silakan ulangi transaksi.", mainKeyboard);
    }
    if (/^\d{6}$/.test(text)) {
      const valid = verifyPin(id, text);
      if (valid === true) {
        delete pendingPinAction[id];
        await bot.sendMessage(id, "✅ PIN benar! Memproses transaksi...");
        return pa.callback(pa.data);
      } else {
        pa.attempts = (pa.attempts || 0) + 1;
        if (pa.attempts >= 3) {
          delete pendingPinAction[id];
          return bot.sendMessage(id, "❌ PIN salah 3x. Transaksi dibatalkan.", mainKeyboard);
        }
        return bot.sendMessage(id, `❌ PIN salah! Sisa percobaan: ${3 - pa.attempts}x`, { reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] } });
      }
    } else if (text.length <= 8) {
      return bot.sendMessage(id, "⚠️ PIN harus 6 digit angka.");
    }
  }

  // ── SET PIN FLOW ──
  if (sessions[id]?.__settingPin) {
    const step = sessions[id].__settingPin;
    if (step === "input") {
      if (!/^\d{6}$/.test(text)) return bot.sendMessage(id, "⚠️ PIN harus 6 digit angka. Coba lagi:");
      sessions[id].__pinTemp = text;
      sessions[id].__settingPin = "confirm";
      return bot.sendMessage(id, "🔐 Konfirmasi PIN kamu (ketik ulang):", { reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] } });
    }
    if (step === "confirm") {
      if (text !== sessions[id].__pinTemp) {
        sessions[id].__settingPin = "input";
        delete sessions[id].__pinTemp;
        return bot.sendMessage(id, "❌ PIN tidak cocok! Masukkan PIN baru:");
      }
      // Simpan PIN
      if (!users[id]) users[id] = {};
      users[id].pin = hashPin(text);
      delete sessions[id].__settingPin;
      delete sessions[id].__pinTemp;
      await saveUser(id);
      return bot.sendMessage(id, "✅ *PIN berhasil diset!*\n\nSetiap transaksi sekarang memerlukan PIN 6 digit kamu.", { parse_mode:"Markdown", ...mainKeyboard });
    }
  }

  // ── UBAH PIN FLOW ──
  if (sessions[id]?.__changingPin) {
    const step = sessions[id].__changingPin;
    if (step === "verify") {
      if (verifyPin(id, text) !== true) return bot.sendMessage(id, "❌ PIN lama salah! Coba lagi:");
      sessions[id].__changingPin = "input";
      return bot.sendMessage(id, "🔐 Masukkan PIN baru (6 digit):");
    }
    if (step === "input") {
      if (!/^\d{6}$/.test(text)) return bot.sendMessage(id, "⚠️ PIN harus 6 digit angka:");
      sessions[id].__pinTemp = text;
      sessions[id].__changingPin = "confirm";
      return bot.sendMessage(id, "🔐 Konfirmasi PIN baru:");
    }
    if (step === "confirm") {
      if (text !== sessions[id].__pinTemp) {
        sessions[id].__changingPin = "input";
        delete sessions[id].__pinTemp;
        return bot.sendMessage(id, "❌ PIN tidak cocok! Masukkan PIN baru lagi:");
      }
      users[id].pin = hashPin(text);
      delete sessions[id].__changingPin;
      delete sessions[id].__pinTemp;
      await saveUser(id);
      return bot.sendMessage(id, "✅ *PIN berhasil diubah!*", { parse_mode:"Markdown", ...mainKeyboard });
    }
  }
...
_Tap untuk copy_`, { parse_mode:"Markdown" });
  ...
  if (data === "cancel_pin") {
    delete pendingPinAction[id];
    if (sessions[id]) { delete sessions[id].__settingPin; delete sessions[id].__changingPin; delete sessions[id].__pinTemp; }
    return bot.sendMessage(id, "❌ Dibatalkan.", mainKeyboard);
  }

  if (data === "pin_set") {
    if (!sessions[id]) sessions[id] = {};
    sessions[id].__settingPin = "input";
    return bot.sendMessage(id, "🔐 *Set PIN Transaksi*\n\nMasukkan PIN 6 digit kamu:\n_Jangan gunakan tanggal lahir atau angka berurutan_", { parse_mode:"Markdown", reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] } });
  }

  if (data === "pin_change") {
    if (!users[id]?.pin) return bot.sendMessage(id, "⚠️ Kamu belum set PIN.", mainKeyboard);
    if (!sessions[id]) sessions[id] = {};
    sessions[id].__changingPin = "verify";
    return bot.sendMessage(id, "🔐 Masukkan PIN lama kamu:", { reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] } });
  }
  ...
  if (data === "pin_remove") {
    ...
    if (!sessions[id]) sessions[id] = {};
    sessions[id].__changingPin = "verify";
    sessions[id].__removingPin = true;
    return bot.sendMessage(id, "🔐 Masukkan PIN kamu untuk menghapus:", { reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] } });
  }

  if (data === "pin_menu") {
    const hasPin = !!users[id]?.pin;
    return bot.sendMessage(id,
      `🔐 *PIN Transaksi*

` +
      `Status: ${hasPin ? "✅ *Aktif*" : "❌ *Belum diset*"}

` +
      `PIN melindungi setiap transaksi pembayaran kamu.`,
      { parse_mode:"Markdown", reply_markup:{ inline_keyboard:[
        hasPin
          ? [{ text:"🔄 Ubah PIN", callback_data:"pin_change" }, { text:"🗑️ Hapus PIN", callback_data:"pin_remove" }]
          : [{ text:"🔐 Set PIN Sekarang", callback_data:"pin_set" }],
...
Nomor: *${nomor}*`,
  ...
  if (data === "ppob_bayar_now" || data.startsWith("ppob_bayar_")) {
    // Cek PIN — kalau belum verified, minta PIN dulu
    if (users[id]?.pin && !sessions[id]?.__pinVerified) {
      pendingPinAction[id] = {
        callback: async () => {
          if (!sessions[id]) sessions[id] = {};
          sessions[id].__pinVerified = true;
          setTimeout(() => { delete sessions[id]?.__pinVerified; }, 30000); // 30 detik
          // Proses transaksi setelah PIN benar
          bot.emit("callback_query", { id: query.id, from: query.from, message: query.message, data });
        },
        data, expiry: Date.now() + 120000, attempts: 0
      };
      return bot.sendMessage(id,
        `🔐 *Masukkan PIN Transaksi*

Ketik 6 digit PIN kamu:`,
        { parse_mode:"Markdown", reply_markup:{ inline_keyboard:[[{text:"❌ Batal",callback_data:"cancel_pin"}]] }}
      );
    }
    if (sessions[id]?.__pinVerified) delete sessions[id].__pinVerified;
    const type  = sessions[id]?.__ppobType;
    ...
    if (users[id]?.pin && !sessions[id]?.__pinVerified) {
      pendingPinAction[id] = {
        callback: async () => {
          if (!sessions[id]) sessions[id] = {};
          sessions[id].__pinVerified = true;
          setTimeout(() => { delete sessions[id]?.__pinVerified; }, 30000);
          bot.emit("callback_query", { id: query.id, from: query.from, message: query.message, data });
        },
        data, expiry: Date.now() + 120000, attempts: 0
      };
...
http.createServer(async (req, res) => {
  ...
  // ══════════════════════════════════════════════════════════
  // DIGIFLAZZ WEBHOOK — Auto-sync produk baru / harga berubah
  // Setup di dashboard Digiflazz → URL: https://DOMAIN/digiflazz-webhook
  // Secret: isi WEBHOOK_SECRET di env (default: "friendlyindonesia")
  // ══════════════════════════════════════════════════════════
  if (req.method === "POST" && path === "/digiflazz-webhook") {
    let body = "";
    req.on("data", chunk => body += chunk);
    req.on("end", async () => {
      try {
        const notif = JSON.parse(body);
        console.log("📦 Digiflazz webhook:", JSON.stringify(notif).substring(0, 300));

        // ── Verifikasi signature Digiflazz ──
        // Digiflazz mengirim header X-Hub-Signature: sha1=HASH
        const sigHeader = req.headers["x-hub-signature"] || "";
        if (WEBHOOK_SECRET && sigHeader) {
          const expected = "sha1=" + crypto.createHmac("sha1", WEBHOOK_SECRET).update(body).digest("hex");
          if (sigHeader !== expected) {
            console.warn("⚠️ Digiflazz webhook signature tidak valid, diabaikan.");
            res.writeHead(403); res.end("Invalid signature");
            return;
          }
        }

        const event = notif.event || notif.type || "";

        // ── Event: produk baru / harga berubah / produk dinonaktifkan ──
        const isProductEvent = /product|price|pricelist/i.test(event);
        if (isProductEvent || !event) {
          // Invalidate cache → digiGetAllProduk() akan re-fetch otomatis
          digiAllProduk   = null;
          digiAllProdukAt = 0;
          console.log("✅ Cache produk Digiflazz di-reset via webhook. Akan di-refresh pada request berikutnya.");

          // Kirim notif ke admin
          if (ADMIN_ID) {
            const produkInfo = notif.data?.product_name || notif.product_name || "";
            const hargaInfo  = notif.data?.price        || notif.price        || "";
            const eventLabel = event || "update";
            try {
              await bot.sendMessage(ADMIN_ID,
                `🔔 *Digiflazz Update Produk*\n\n` +
                `📦 Event  : \`${eventLabel}\`\n` +
                (produkInfo ? `📝 Produk : *${produkInfo}*\n` : "") +
                (hargaInfo  ? `💰 Harga  : *Rp ${Number(hargaInfo).toLocaleString("id-ID")}*\n` : "") +
                `\n✅ Cache produk otomatis di-refresh!`,
                { parse_mode: "Markdown" }
              );
            } catch(e) { /* admin mungkin belum set */ }
          }
        }

        // ── Event: status transaksi (sukses/gagal dari Digiflazz) ──
        const isTxEvent = /transaction|trx/i.test(event) || notif.data?.ref_id;
        if (isTxEvent) {
          const txData  = notif.data || notif;
          const refId   = txData.ref_id   || txData.ref_id   || "";
          const status  = txData.status   || "";
          const sn      = txData.sn       || "";
          const message = txData.message  || "";

          console.log(`📬 Digiflazz TX update: refId=${refId} status=${status} sn=${sn}`);

          // Cari user berdasarkan ref_id di pendingTx
          const matchOrder = Object.entries(pendingTx).find(([oid]) => oid === refId);
          if (matchOrder) {
            const [orderId, tx] = matchOrder;
            const userId = tx.userId;

            if (status === "Sukses" && tx.status !== "success") {
              tx.status = "success";
              // Potong saldo kalau belum (untuk kasus pending → sukses)
              if (!tx.saldoDipotong) {
                tx.saldoDipotong = true;
                await kurangiSaldo(userId, tx.amount);
                const pts = getPoints(tx.amount);
                if (users[userId]) { users[userId].points = (users[userId].points||0) + pts; await saveUser(userId); }
                await addTxHistory(userId, { type: tx.productName || tx.type, amount: tx.amount, order_id: orderId, points: pts, status: "success", sn, date: new Date().toLocaleDateString("id-ID") });
              }
              delete pendingTx[orderId];
              try {
              ...
              if (tx.saldoDipotong) {
                ...
                await addTxHistory(userId, { type: `Refund - ${tx.productName || tx.type}`, amount: tx.amount, order_id: orderId, status: "refund", date: new Date().toLocaleDateString("id-ID") });
              }
              delete pendingTx[orderId];
              try {
                await bot.sendMessage(userId,
                  `❌ *Transaksi Gagal*\n\n` +
                  `📦 *${tx.productName || tx.type}*\n` +
                  `${message ? `📋 Alasan: ${message}\n` : ""}` +
                  `${tx.saldoDipotong ? `\n💸 Saldo sudah dikembalikan!` : "Saldo tidak dipotong."}`,
                  { parse_mode: "Markdown", ...mainKeyboard }
                );
              } catch(e) {}
            }
...
bot.onText(/\/admin/, async (msg) => {
  ...
  await bot.sendMessage(msg.from.id,
    ...
    `_Ketik /syncproduk untuk sync produk Digiflazz_\n` +
    `_Ketik /webhookdigi untuk setup auto-sync_`,
    { parse_mode:"Markdown" }
  );
});
...
// ── SYNC PRODUK MANUAL & INFO WEBHOOK DIGIFLAZZ ──────────────
bot.onText(/\/syncproduk/, async (msg) => {
  if (String(msg.from.id) !== String(ADMIN_ID)) return;
  await bot.sendMessage(msg.from.id, "🔄 Memuat ulang semua produk Digiflazz...");
  digiAllProduk   = null;
  digiAllProdukAt = 0;
  try {
    const produk = await digiGetAllProduk();
    const cats   = [...new Set(produk.map(p => p.category))].sort();
    await bot.sendMessage(msg.from.id,
      `✅ *Sync Produk Selesai!*\n\n` +
      `📦 Total produk : *${produk.length}*\n` +
      `🗂️ Kategori     : *${cats.length}*\n\n` +
      `_${cats.join(", ")}_`,
      { parse_mode: "Markdown" }
    );
  } catch(e) {
    await bot.sendMessage(msg.from.id, `❌ Gagal sync: ${e.message}`);
  }
});

bot.onText(/\/webhookdigi/, async (msg) => {
  if (String(msg.from.id) !== String(ADMIN_ID)) return;
  const domain = process.env.RAILWAY_PUBLIC_DOMAIN
    || process.env.WEBHOOK_DOMAIN
    || "YOUR_DOMAIN";
  const webhookUrl = `https://${domain}/digiflazz-webhook`;
  await bot.sendMessage(msg.from.id,
    `🔗 *Setup Webhook Digiflazz*\n\n` +
    `*URL Webhook:*\n\`${webhookUrl}\`\n\n` +
    `*Langkah-langkah:*\n` +
    `1. Login ke dashboard Digiflazz\n` +
    `2. Menu *Pengaturan → Webhook / Callback*\n` +
    `3. Paste URL di atas\n` +
    `4. Isi Secret: \`${WEBHOOK_SECRET}\`\n` +
    `5. Centang event: *Pricelist Update* & *Transaksi*\n` +
    `6. Klik Simpan\n\n` +
    `_Setelah disimpan, produk baru akan auto-sync!_ ✅\n\n` +
    `_Atau gunakan /syncproduk untuk sync manual._`,
    { parse_mode: "Markdown" }
  );
});
