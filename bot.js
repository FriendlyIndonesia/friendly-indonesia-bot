...
async function digiGetProduk(kategori) {
  if (digiProdukCache[kategori] && Date.now() - digiProdukCacheAt < 3600000) {
    return digiProdukCache[kategori];
  }
  try {
    const sign = digiSign(DIGI_USER, DIGI_KEY(), "pricelist");
    const payload = JSON.stringify({ cmd:"prepaid", username:DIGI_USER, sign });
    const res = await fetchJSON("https://api.digiflazz.com/v1/price-list", {
      method:"POST", headers:{"Content-Type":"application/json"}, body:payload
    });
    if (res?.data) {
      const cats = [...new Set(res.data.map(p => p.category))];
      console.log("📦 Semua kategori Digiflazz:", cats.join(", "));

      // Map nama kategori bot ke regex untuk match Digiflazz
      const katRegex = {
        "pulsa":    /pulsa/i,
        "data":     /data/i,
        "pln":      /pln|listrik/i,
        "bpjs":     /bpjs/i,
        "game":     /game|voucher/i,
        "ewallet":  /e.?money|wallet|gopay|ovo|dana/i,
        "tv":       /tv|televisi/i,
        "internet": /internet|wifi/i,
      };
      const regex = katRegex[kategori.toLowerCase()] || new RegExp(kategori, "i");
      const matchCats = cats.filter(c => regex.test(c));
      console.log(`🔍 "${kategori}" match: [${matchCats.join(", ")}]`);

      const filtered = res.data.filter(p =>
        matchCats.includes(p.category) &&
        p.seller_product_status === true
      );
      console.log(`✅ ${filtered.length} produk ditemukan untuk "${kategori}"`);
      digiProdukCache[kategori] = filtered;
      digiProdukCacheAt = Date.now();
      return filtered;
    }
    console.error("digiGetProduk invalid response:", JSON.stringify(res).substring(0,200));
    return [];
  } catch(e) { console.error("digiGetProduk error:", e.message); return []; }
}
...
bot.on("message", async (msg) => {
  ...
  if (sessions[id]?.__ppobStep === "input_nomor") {
    ...
    return bot.sendMessage(id,
      `📱 *Pilih Produk ${svcNames[type] || type}*\nNomor: *${nomor}*\n\nPilih nominal:`,
      { parse_mode:"Markdown", reply_markup:{ inline_keyboard: btns } }
    );
  }
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("ppob_pick_")) {
    ...
    if (!type || !nomor) return bot.sendMessage(id, "⚠️ Session expired. Ketik nomor HP lagi.", mainKeyboard);
    ...
    let produkList = sessions[id]?.__ppobProduk;
    if (!produkList?.length) produkList = await digiGetProduk(digiKats[type] || type);
