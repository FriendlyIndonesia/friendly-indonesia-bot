...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("pX.")) {
    ...
    // Cari produk by SKU dari cache digiflazz
    ...
    // Filter produk berdasarkan brand e-wallet
    const ewalletBrandFilter = {
      ewallet_dana:   p => /\bdana\b/i.test(p.product_name),
      ewallet_gopay:  p => /gopay/i.test(p.product_name),
      ewallet_ovo:    p => /\bovo\b/i.test(p.product_name),
      ewallet_shopee: p => /shopee/i.test(p.product_name),
    };
    if (ewalletBrandFilter[type]) {
      const filtered = allProds.filter(ewalletBrandFilter[type]);
