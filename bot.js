...
async function digiGetProduk(kategori) {
  ...
  try {
    ...
    if (res?.data) {
      // Log semua kategori unik untuk debug
      const cats = [...new Set(res.data.map(p => p.category))];
      console.log("📦 Kategori Digiflazz tersedia:", cats.join(", "));
      const filtered = res.data.filter(p =>
        p.category?.toLowerCase().includes(kategori.toLowerCase()) &&
        p.seller_product_status === true
      );
      console.log(`🔍 Filter "${kategori}": ${filtered.length} produk ditemukan dari ${res.data.length} total`);
      digiProdukCache[kategori] = filtered;
      digiProdukCacheAt = Date.now();
      return filtered;
    }
    console.error("digiGetProduk: response tidak valid:", JSON.stringify(res).substring(0,200));
    return [];
  } catch(e) { console.error("digiGetProduk:", e.message); return []; }
}
